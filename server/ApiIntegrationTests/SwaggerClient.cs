...
namespace Generated
{
                ...
                using (var request_ = new System.Net.Http.HttpRequestMessage())
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            var responseData_ = response_.Content == null ? null : await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            throw new ApiException("The HTTP status code of the response was not expected (" + status_ + ").("+data.Email+ " "+data.Password+")", status_, responseData_, headers_,null);
                        }
                    }
                    finally
                    {
    ...
    #pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual Task<SwaggerResponse<ValueTupleOfCurrentBalanceDtoAndListOfAutomatedTicketsDto>>
            CreateGameTicketAsync(List<Service.TransferModels.Requests.CreateTicketDto> tickets)
        {
        ...
        /// <param name="cancellationToken">A cancellation token that can be used by other objects or threads to receive notice of cancellation.</param>
        /// <exception cref="ApiException">A server side error occurred.</exception>
        public virtual async Task<SwaggerResponse<ValueTupleOfCurrentBalanceDtoAndListOfAutomatedTicketsDto>>
            CreateGameTicketAsync(List<Service.TransferModels.Requests.CreateTicketDto> tickets,
    ...
    {
        // Assuming the class has the properties for the tuple items
        public CurrentBalanceDto Item1 { get; set; }  // The first item in the tuple (CurrentBalanceDto)
        public List<AutomatedTicketsDto> Item2 { get; set; }  // The second item in the tuple (List of AutomatedTicketsDto)

        // Implementing the Deconstruct method
        public void Deconstruct(out CurrentBalanceDto currentbalance, out List<AutomatedTicketsDto> automatedtickets)
        {
            // Assign the tuple items to the out parameters
            currentbalance = Item1;  // Assign CurrentBalanceDto to currentbalance
            automatedtickets = Item2;  // Assign List<AutomatedTicketsDto> to automatedtickets
        }
    }
